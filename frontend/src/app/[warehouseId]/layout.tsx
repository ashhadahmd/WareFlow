    ...
    <div className="min-h-screen flex bg-[#f8fafc] dark:bg-background">
      ...
      <div className="lg:hidden fixed top-0 left-0 w-full h-16 bg-white/80 dark:bg-card/80 backdrop-blur-md border-b border-gray-200 dark:border-border z-[60] flex items-center justify-between px-4">
        <h1 className="text-lg font-bold flex items-center gap-2">
          <span className="bg-primary text-white p-1 rounded text-xs">WH</span>
          WareFlow
        </h1>
        <button 
      ...
      {/* --- Sidebar (Fixed responsiveness) --- */}
      <div className={cn(
        "w-64 fixed inset-y-0 z-[59] flex flex-col glass-panel rounded-none border-y-0 border-l-0 transition-transform duration-300 lg:translate-x-0",
        isSidebarOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        <div className="p-6">
          <h1 className="text-xl font-bold flex items-center gap-2">
            <span className="bg-primary text-white p-1 rounded">WH</span>
            WareFlow
          </h1>
        </div>
        ...
        <div className="px-4 pb-4">
          <button 
             ...
             className="w-full text-left p-3 rounded-xl border border-gray-200 bg-white/50 dark:bg-white transition-colors flex items-center justify-between group"
          >
            <div className="overflow-hidden">
               <p className="text-xs text-gray-500 font-medium">WORKSPACE</p>
               <p className="font-semibold text-gray-900 truncate dark:text-black">{warehouseName}</p>
            </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive 
                    ? 'bg-black/5 text-foreground border-l-4 border-l-primary dark:bg-foreground/10' 
                    : 'text-gray-500 hover:bg-black/5 dark:hover:text-foreground border-l-4 border-l-transparent'
                )}
              >
                <item.icon size={20} className={isActive ? 'text-primary' : 'text-foreground'} />
                {item.name}
              </Link>
            );
          })}
        ...
        <div className="p-4 mt-auto space-y-2">
          ...
          <button 
            ...
            className="flex items-center gap-3 px-3 py-2.5 w-full rounded-lg font-medium text-gray-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30 dark:hover:text-red-400 transition-colors"
          >
            <LogOut size={20} />
            Logout
          </button>
